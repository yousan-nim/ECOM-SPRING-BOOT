...
up-local: env-local ## Start 3 Postgres + pgAdmin (no apps; run apps from IDE)
	$(LOCAL) up -d postgres-catalog postgres-order postgres-payment pgadmin

up-local-full: env-local ## Start everything incl. all 3 services + gateway in containers
...
migrate-all: migrate-catalog migrate-order migrate-payment ## Apply migrations for all services
...
tables: ## List tables in all 3 databases
...
.PHONY: help up-local up-local-full down-local logs-local up-dev down-dev logs-dev up-prod down-prod \
        build test build-catalog build-order build-payment \
        migrate-catalog migrate-order migrate-payment migrate-all \
        psql-catalog psql-order psql-payment tables db-reset \
